import React from 'react';
import { Image, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
import Icon  from 'react-native-vector-icons/MaterialCommunityIcons';
import { stylesGlobal } from '../../global/styles';

export default ({drawer}:any) => {
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <TouchableOpacity onPress={() => drawer.current.closeDrawer()}>
                    <Icon name="chevron-left" size={25} color='#fff'/>
                </TouchableOpacity>
                <Text style={styles.headerTitle}>Meu perfil</Text>
                <TouchableOpacity>
                    <Icon name="account-edit-outline" size={25} color='#fff'/>
                </TouchableOpacity>
            </View>
            <View style={styles.profile}>
                <View style={styles.photoBox}>
                    <Image style={styles.photo} source={{uri: 'data:image/png;base64,'}}/>
                </View>
                <Text style={styles.name}>Eduardo da Silva</Text>
            </View>
            <View style={styles.content}>
                <View style={styles.cardBox}>
                    <View style={styles.card}>
                        <Text style={[styles.cardTextBold, styles.cardText]}>7</Text>
                        <Text style={[styles.cardTextSmall, styles.cardText]}>Trabalhos</Text>
                        <Text style={[styles.cardTextSmall, styles.cardText]}>Última semana</Text>
                    </View>
                    <View style={styles.card}>
                        <Text style={[styles.cardTextBold, styles.cardText]}>4.92
                            <Icon name="star" size={20} color="#fff"/>
                        </Text>
                        <Text style={[styles.cardTextSmall, styles.cardText]}>Avaliação</Text>
                    </View>
                </View>
            </View>
            <View style={styles.navigation}>
                <View style={styles.btn}>
                    <Text style={styles.btnTitle}>Serviços publicados</Text>
                    <Icon name="chevron-right" size={20} color="#000"/>
                </View>
                <View style={styles.btn}>
                    <Text style={styles.btnTitle}>Serviços cancelados</Text>
                    <Icon name="chevron-right" size={20} color="#000"/>
                </View>
                <View style={styles.btn}>
                    <Text style={styles.btnTitle}>Serviços efetuados</Text>
                    <Icon name="chevron-right" size={20} color="#000"/>
                </View>
                <View style={styles.btn}>
                    <Text style={styles.btnTitle}>Minha carteira</Text>
                    <Icon name="chevron-right" size={20} color="#000"/>
                </View>
            </View>
            <View style={styles.footer}>
                <Text style={styles.footerText}>Help-me</Text>
            </View>
        </View>
    )
}

const styles = StyleSheet.create({
    container: {
        flex: 1,
        alignItems: 'center',
        backgroundColor: '#fff',
    },
    header:{ 
        paddingHorizontal: 20,
        flex: .3,
        width: '100%',
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent:'space-between',
        backgroundColor:  stylesGlobal.colors.primary.orange
    },
    headerTitle: {
        fontSize: 20,
        fontWeight: '600',
        color: '#fff'
    },
    profile: {  
        marginTop: 20,
        flex: 1,
        width: '100%',
        alignItems: 'center',
    },
    photoBox: {
        width: 100,
        height: 100,
        borderRadius: 50,
        overflow: 'hidden',
    },
    photo: {
        width: '100%',
        height: '100%',
        resizeMode:'cover'
    },
    name: {
        margin: 15,
        color: stylesGlobal.colors.primary.gray,
        fontSize: 20,
        fontWeight: '600'
    },
    content: {
        width: '100%',
        flex: 1,
        paddingHorizontal: 20,
    },
    cardBox: {
        width: '100%',
        height: 100,
        borderRadius: 10,
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'space-between',
        backgroundColor: stylesGlobal.colors.primary.orange
    },
    card: {
        alignItems: 'center',
        justifyContent: 'center',
        flex: 1,
    },
    cardTextBold: {
        fontSize:20,
        fontWeight: '800'
    },
    cardTextSmall: {
        fontSize:16,
        fontWeight: '400'
    },
    cardText: {
        color: '#fff'
    },
    navigation:{
        flex: 1.5,
        width: '100%',
        alignItems: 'center',
        justifyContent: 'space-between',
        paddingHorizontal: 20,
    },
    btn: {
        borderRadius: 10,
        padding: 10,
        width: '100%',
        height: 50,
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'space-between',
        backgroundColor: '#fff',
        elevation: 1,
    },
    btnTitle: {
        fontSize: 18,
        color: stylesGlobal.colors.primary.gray
    },
    footer: {
        flex: .5,
        alignItems: 'center',
        justifyContent: 'center'
    },
    footerText: {
        fontSize: 40,
        fontWeight: '700',
        color: stylesGlobal.colors.primary.orange
    }
})